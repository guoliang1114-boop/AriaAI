...
def upgrade():
    ...
    if "clientstakeholder" not in tables:
        op.create_table(
            "clientstakeholder",
    ...
    indexes = {index["name"] for index in inspector.get_indexes("clientstakeholder")}
    if "ix_clientstakeholder_client_id" not in indexes:
...
def downgrade():
    bind = op.get_bind()
    inspector = inspect(bind)
    if "clientstakeholder" not in inspector.get_table_names():
        return
    indexes = {index["name"] for index in inspector.get_indexes("clientstakeholder")}
    if "ix_clientstakeholder_client_id" in indexes:
