...
export function Welcome() {
  ...
  const recentClients = useMemo(
    () =>
      [...clients]
        .sort((a, b) => new Date(b.client_memory_updated_at || b.created_at).getTime() - new Date(a.client_memory_updated_at || a.created_at).getTime())
        .slice(0, 4),
    [clients],
  )
  ...
  if (loading) {
    return <div className="flex h-full items-center justify-center bg-surface"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div>
  }
      ...
      <>
        ...
        <div className="flex h-full items-center justify-center bg-surface">
          <div className="max-w-md rounded-2xl border border-outline bg-surface p-8 text-center shadow-sm">
            ...
            <button onClick={() => void loadData()} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-medium text-white transition hover:bg-primary/90">
      ...
      <div className="h-full overflow-auto bg-surface">
        <div className="space-y-6 px-8 py-8">
          <section className="rounded-3xl bg-gradient-to-br from-primary via-indigo-600 to-indigo-700 p-8 text-white shadow-sm">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
              <div className="max-w-2xl">
                ...
                <h1 className="mb-3 text-3xl font-bold tracking-tight">{user?.display_name || (isZh ? '欢迎回来' : 'Welcome back')}</h1>
                <p className="text-sm leading-6 text-white/85">
                  {isZh ? '这里是今天的工作台。先看待办、项目节奏和记忆健康度，再决定优先推进哪件事。' : 'Use this workspace to review today’s priorities, project momentum, and memory health before you move work forward.'}
                </p>
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                {[
                  { title: isZh ? '开始新对话' : 'Start a chat', sub: isZh ? '进入 AI 工作台' : 'Open the AI workspace', icon: Sparkles, path: '/chat', primary: true },
                  { title: isZh ? '新建项目' : 'Create project', sub: isZh ? '建立新的交付空间' : 'Open a new delivery workspace', icon: FolderKanban, path: '/projects/new' },
                  { title: isZh ? '项目记忆' : 'Project memory', sub: isZh ? '检查项目记忆和摘要缓存' : 'Review project memory and cached summaries', icon: Brain, path: '/settings/memory' },
                  { title: isZh ? '客户记忆' : 'Client memory', sub: isZh ? '查看客户长期经验沉淀' : 'Review client-level knowledge', icon: Users, path: '/settings/client-memory' },
                ].map((item) => (
                  <button key={item.title} onClick={() => navigate(item.path)} className={`flex items-center justify-between rounded-2xl px-4 py-3 text-left transition ${item.primary ? 'bg-white text-primary hover:bg-white/95' : 'border border-white/20 bg-white/10 hover:bg-white/15'}`}>
                    <div>
                      <div className="text-sm font-semibold">{item.title}</div>
                      <div className={`mt-1 text-xs ${item.primary ? 'text-primary/70' : 'text-white/70'}`}>{item.sub}</div>
                    </div>
          ...
          <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {[
              { label: isZh ? '活跃项目' : 'Active projects', value: activeProjects.length, sub: isZh ? `总计 ${projects.length} 个项目` : `${projects.length} total projects`, icon: FolderKanban },
              { label: isZh ? '活跃客户' : 'Active clients', value: activeClients.length, sub: isZh ? `总计 ${clients.length} 个客户` : `${clients.length} total clients`, icon: Building2 },
              { label: isZh ? '合同总额' : 'Contract value', value: formatCurrency(contractValue, isZh), sub: isZh ? '按未归档项目统计' : 'Across non-archived projects', icon: Wallet },
              { label: isZh ? '技能数量' : 'Available skills', value: skills.length, sub: isZh ? '可复用的工作流和技能' : 'Reusable skills and workflows', icon: Sparkles },
            ].map((card) => (
          ...
          <section className="grid gap-6 lg:grid-cols-[1.2fr_1fr]">
            <div className="rounded-2xl border border-outline bg-surface p-6 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-on-surface-muted">{isZh ? '把真正需要你推进的动作拉到最前面。' : 'Pull the actions that need your attention to the top.'}</p>
                </div>
                <button onClick={() => navigate('/projects')} className="inline-flex items-center gap-1 text-sm font-medium text-primary hover:underline">
                  {isZh ? '查看项目' : 'Open projects'} <ArrowRight className="h-4 w-4" />
                </button>
              </div>
              <div className="grid gap-3 sm:grid-cols-3">
                <button onClick={() => navigate('/projects')} className="rounded-2xl border border-outline px-4 py-4 text-left transition hover:bg-surface-container-low">
                ...
                <button onClick={() => navigate('/chat')} className="rounded-2xl border border-outline px-4 py-4 text-left transition hover:bg-surface-container-low">
              ...
              <div className="mt-4 space-y-3">
                {myTodos.slice(0, 4).map((todo) => (
                  <button key={todo.id} onClick={() => navigate(`/projects/${todo.project_id}/todos`)} className="flex w-full items-start gap-3 rounded-2xl border border-outline px-4 py-3 text-left transition hover:bg-surface-container-low">
                ...
                {myTodos.length === 0 ? <div className="rounded-2xl bg-surface-container-low px-4 py-6 text-center text-sm text-on-surface-muted">{isZh ? '目前没有分配给你的项目待办。' : 'No project todos are assigned to you right now.'}</div> : null}
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-outline bg-surface p-6 shadow-sm">
              <div className="mb-4">
                ...
                <p className="mt-1 text-sm text-on-surface-muted">{isZh ? '首页只展示健康概览，具体管理统一进设置。' : 'Use the dashboard for health signals and settings for full management.'}</p>
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                {[
                  ...
                  <button key={item.label} onClick={() => navigate(item.path)} className="rounded-2xl border border-outline px-4 py-4 text-left transition hover:bg-surface-container-low">
            ...
            <div className="rounded-2xl border border-outline bg-surface p-6 shadow-sm">
              ...
              <div className="mt-4 space-y-3">
                {['lead', 'opportunity', 'won', 'delivering', 'archived'].map((status) => {
                  ...
                  return (
                    <div key={status} className="rounded-2xl bg-surface-container-low px-4 py-3">
                      <div className="flex items-center justify-between text-sm">
                        <span className="font-medium text-on-surface">{getStageLabel(status as Project['status'], isZh)}</span>
                        <span className="text-on-surface-muted">{count}</span>
                      </div>
                      <div className="mt-2 h-2 overflow-hidden rounded-full bg-surface-container-high">
                        <div className="h-full rounded-full bg-primary" style={{ width: `${projects.length ? (count / projects.length) * 100 : 0}%` }} />
                      </div>
                    </div>
                  )
                })}
            ...
            <div className="rounded-2xl border border-outline bg-surface p-6 shadow-sm">
              ...
              <div className="mt-4 space-y-2">
                {recentProjects.map((project) => (
                  <button key={project.id} onClick={() => navigate(`/projects/${project.id}`)} className="flex w-full items-center justify-between rounded-2xl px-3 py-3 text-left transition hover:bg-surface-container-low">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {recentClients.map((client) => (
                      <button key={client.id} onClick={() => navigate(`/clients/${client.id}`)} className="flex w-full items-center justify-between rounded-2xl px-3 py-3 text-left transition hover:bg-surface-container-low">
                        <div className="min-w-0">
                          <div className="truncate text-sm font-medium text-on-surface">{client.name}</div>
                          <div className="mt-1 text-xs text-on-surface-muted">{client.industry || (isZh ? '未填写行业' : 'No industry')} · {client.project_names.length} {isZh ? '个项目' : 'projects'}</div>
                        </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {recentConversations.map((conversation) => (
                      <button key={conversation.id} onClick={() => navigate('/chat')} className="flex w-full items-center justify-between rounded-2xl px-3 py-3 text-left transition hover:bg-surface-container-low">
                        <div className="min-w-0">
