      ...
      <div className="min-h-full bg-surface">
        <div className="mx-auto max-w-5xl space-y-6 px-6 py-8">
          ...
          {!loading && messages.length === 0 ? (
            <div className="rounded-3xl border border-outline/10 bg-surface-container-low px-6 py-16 text-center">
          ...
          {!loading ? (
            <div className="space-y-4">
              {messages.map((message) => (
                <div
                  key={message.id}
                  className={`rounded-3xl border px-5 py-5 shadow-sm transition ${
                    message.is_read ? 'border-outline/10 bg-surface-container-low' : 'border-primary/15 bg-white'
                  }`}
                >
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0 flex-1">
                      <div className="mb-3 flex flex-wrap items-center gap-2">
                        <span className={`rounded-full border px-2.5 py-1 text-xs font-medium ${getLevelClasses(message.level)}`}>
                          {message.level}
                        </span>
                        <span
