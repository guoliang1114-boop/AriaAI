...
Updated 2026-09-22. Start with [README](../README.md) for setup, [CONTRIBUTING](../CONTRIBUTING.md) for checks, and [agent.md](../agent.md) for repository boundaries. Product and runtime state belong to Aria's native services.

Status labels describe the document, not a released version: **Contract** documents implemented behavior with historical sections; **Plan** describes direction and may include completed phases; **Historical** records an earlier iteration; **Current work** covers changes still awaiting release. Use [the acceptance record](23-2026-09-19全量推进验收.md), [memory boundary acceptance](26-模型完成状态与记忆生成加固验收.md), [Run Harness compatibility acceptance](27-Run-Harness合同兼容性验收.md), and [source injection acceptance](28-摘要来源边界与注入对抗验收.md) for actual validation and deployment evidence.
