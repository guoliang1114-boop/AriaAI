...
### Phase 4O：模型记忆 schema 输入封口（已实施）

- 完整项目/客户记忆解析只接收 Aria 声明的业务槽位，并按字符串、字符串数组和对象数组规范化类型；模型额外根键、伪造的 `_last_failure` 等运维键、版本元数据及错误类型项不会进入持久化输入。
- 成功保存同步清除原生失败状态和旧聚合兼容回执，避免原生清空后兼容读取复活旧错误。部分重建不静默删除未确认的历史键，等待指纹定位后再治理。
- 确定性门禁为 128 个场景、35 项指标；本阶段无迁移、不运行或连接 Codex。

### Phase 4P：历史拍平干系人无损隔离（已实施）

- Phase 4N 的七个未知键指纹已精确对应旧模型把一个 `structured_stakeholders` 对象拍平到客户记忆根层的历史形态。迁移 `049_v1_49` 只匹配七键齐全且值全部为字符串的记录，在一个事务中先写私有隔离列、再移除活跃根键；部分形态、异常类型和非法 JSON 均不改动。
- 隔离条目保留原始值但不参与 API 序列化，也不自动晋升为正式联系人。降级只补回活跃 JSON 中不存在的键，不覆盖后来产生的新值；数据库备份仍是迁移硬前置条件。
- 部署报告只输出隔离实体、条目、已识别、未知及异常计数，不返回键值、实体 ID 或正文。确定性门禁为 129 个场景、35 项指标，不运行、导入或连接 Codex。

### Phase 4Q：重建历史原生化与聚合 envelope 退出（已实施）

- 幂等迁移 `050_v1_50` 将项目和客户最近 10 次重建历史搬入 owner 私有原生列。只有旧值为合法数组、且原生值为空或一致时才从活跃聚合移除；原生/旧值分歧、非法 JSON 和异常类型均保留旧值并阻止静默清理。
- `memory_version`、`last_updated_at`、`stale` 已长期由 owner 原生列作为权威，本阶段从当前项目/客户聚合 JSON 停止持久化；公共 API 继续从 owner 行回填原字段，前端、Prompt 和调用方合同不变。历史 `ProjectMemorySnapshot` / `ClientMemorySnapshot` 不批量改写。
- 新重建在同一 owner 行锁事务内写原生历史与业务槽位，并用只含业务槽位和仍待迁移元数据的兼容聚合创建新快照。任务中心、槽位 API 和重建模式展示改读原生历史；失败、取消、候选、晋升与回滚语义保持不变。
- `Memory Read Authority Report v2` 使用真实持久化 JSON 判断聚合专属键，不再把 API 临时补齐的 owner 字段误计为存储依赖；非法或非对象存储会显式失败关闭。operation-state v2 报告增加重建历史的原生覆盖、缺失和分歧计数，仍不返回历史正文、实体 ID 或业务值。
- 降级只补回当前聚合中缺失的 envelope 字段，不覆盖后来产生的新值。数据库备份仍是生产迁移硬前置条件；确定性门禁为 130 个场景、35 项指标，不运行、导入或连接 Codex。

### Phase 4R：记忆运维状态单一权威与旧副本退役（已实施）

- 幂等迁移 `051_v1_51` 清理项目失败回执、客户晋升回执，以及客户失败回执和重建取消代次的旧聚合副本。只有旧值类型合法，且原生列为空可先无损回填或与旧值完全一致时才移除；原生值非法、双方分歧、旧值类型异常、代次超长和非法聚合 JSON 均原样保留。
- 运行期读取改为 owner 原生列单一权威，不再从聚合 JSON 回退失败、晋升、取消代次或重建历史。所有成功、失败、取消和晋升写路径停止 legacy 双写，并在公共记忆投影与新持久化聚合中统一剥离私有运维键；项目权限、owner 行锁、CAS、HITAS 和任务语义不变。
