...
补充进展（2026-09-04）：Phase 4O 将完整项目/客户记忆解析限制到 Aria 声明的业务槽位，模型返回的额外根键、伪造运维键或版本元数据不再进入持久化输入；成功保存同步清除原生与兼容聚合失败回执，避免旧回退复活已恢复的错误。部分重建暂不静默删除历史未知键，待 Phase 4N 指纹确认后治理。确定性门禁为 128 个场景、35 项指标；本阶段无迁移，不引入或连接 Codex。
