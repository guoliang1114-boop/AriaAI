...
第十一批继续把槽位拆成可独立审计的事实账本。`ProjectMemoryFact` / `ClientMemoryFact` 使用内容寻址身份，分别记录首次/最近出现版本、active/retired 生命周期、事实内容摘要、独立新鲜度和有界来源关系；`matched` 仅表示来源标签可确定性命中事实，`scoped` 只表示重建槽位时读过该来源，历史聚合与无来源分别标记 `legacy` / `unresolved`，不得冒充直接验证。问答时 `M*` Manifest、引用和 Context Receipt 带事实身份、状态与聚合计数，项目/客户记忆页逐条展示。历史回填由幂等 `034_v1_34` 管理，确定性门禁扩展为 56 场景；仍不引入 Codex 运行时、SDK、协议或通信。
...
补充进展（2026-09-03）：Phase 4F 将 full-history Mode 的预预算候选从 24 条扩展为 96 条，并让 `MODE_CONFIG.history_strategy/history_window` 真正统一控制 full/recent/none。统一预算器按最近轮次、工具批次和有界较早摘录压缩，Context Receipt 明确区分加载、保留、摘要化和近期截短数量。`ChatTrace` 在原始内部记录之外新增内容安全诊断投影、分页和同会话两轮对比，项目消息按需展示路由、上下文、执行和差异，不返回 Prompt、正文、工具 payload、路径或隐藏推理。确定性门禁扩展为 112 个场景、32 项指标。本阶段复用现有表，不新增迁移，也不引入 Codex 运行时、SDK、协议或通信。
...
补充进展（2026-09-03）：Phase 4H 针对真实 Provider 抽测暴露的“相关指标正确但遗漏用户指定维度”问题，将逐项覆盖清单、当前直接证据优先和未核验记忆限定写入文件化回答纪律。真实 Provider 门禁扩展为 6 个合成用例，事实/引用/拒答/来源优先/溯源校准全部要求 1.0，不支持主张要求 0；首答单独计分，失败项可进入最多两轮只反馈 `[R*]` 缺项和引用格式的定向修复，最终门禁不降级。新增确定性 `grounded_answer_contract_accuracy`，发布基线为 120 个场景、34 项指标。记忆界面显示来源可核验比例与待补证事实；无数据库迁移，不引入或连接 Codex。

补充进展（2026-09-03）：Phase 4I 为聚合记忆 JSON 的渐进废弃建立 `Memory Read Authority Report v1`。报告以槽位名和计数量化账本覆盖、缺失/损坏回退、双写差异与聚合专属元数据，不包含记忆事实或来源正文。项目/客户记忆 UI 展示当前读取权威与回退/差异数；部署和生产数据库 E2E 运行内容安全的只读全库汇总。确定性门禁为 124 个场景、35 项指标；本阶段不根据报告自动删除兼容数据，不新增迁移，不引入或连接 Codex。

补充进展（2026-09-04）：Phase 4J 先将双写差异按固定槽位、JSON 类型和版本关系做无正文分类，生产证据确认 21 个差异全部为 `033_v1_33` 遗留的版本一致 `null -> array` 占位。`047_v1_47` 仅在原聚合键缺失、摘要合法且版本一致时补齐 `[]`，显式空值、内容、损坏记录和版本冲突全部跳过。迁移不改写历史快照，由自动部署在生产备份验证后执行，不调用模型。

补充进展（2026-09-04）：Phase 4K 在生产业务槽位零回退、零差异后，将记忆详情 API、项目简报、干系人分析和工作区/客户项目组合上下文切换为经摘要验证的槽位值。缺失或损坏只回退对应聚合槽位，工作区批量查询避免新增 N+1；写入、快照、回滚、候选和晋升继续保留聚合兼容语义。本阶段无迁移，不运行或连接 Codex。

补充进展（2026-09-04）：Phase 4L 将 `_client_promotion`、`_last_failure` 等 Aria 运维键按项目/客户作用域加入内容安全分类，避免把已知元数据误报成未知业务键，同时防止跨作用域键被全局白名单掩盖。报告仍不返回元数据值，且存在任何聚合专属键时容器退役准备度仍为 false。确定性门禁为 125 个场景、35 项指标；本阶段无迁移。

补充进展（2026-09-04）：Phase 4M 通过 `048_v1_48` 将项目/客户失败回执、项目到客户晋升回执和客户重建取消代次回填到 owner 原生运维列。运行期在既有授权与 owner 行锁事务内双写，读取以原生列为权威并对未迁移记录保留聚合回退；成功、失败、取消和并发冲突语义不变。部署报告新增无正文 operation-state 覆盖/缺失/分歧计数，确定性门禁为 126 个场景、35 项指标。聚合副本和历史快照尚未删除，不引入 Codex 运行时、SDK、协议或通信。

补充进展（2026-09-04）：Phase 4N 针对生产中仍存在于一个客户的 7 个未分类聚合键增加隐私安全诊断。报告只输出带 Aria 命名空间的键 SHA-256、长度、JSON 类型和聚合计数，不输出键名、值、实体 ID 或正文；诊断按实体最多保留 64 项、汇总最多保留 128 项，并显式标记截断。后续可与代码内历史 schema 候选离线比对，未确认前继续阻止容器退役。确定性门禁为 127 个场景、35 项指标；本阶段只读、无迁移，不引入或连接 Codex。

补充进展（2026-09-04）：Phase 4O 将完整项目/客户记忆解析限制到 Aria 声明的业务槽位，并按字符串、字符串数组和对象数组规范化槽位类型；模型返回的额外根键、伪造运维键、版本元数据或错误类型项不再进入持久化输入。成功保存同步清除原生与兼容聚合失败回执，避免旧回退复活已恢复的错误。部分重建暂不静默删除历史未知键，待 Phase 4N 指纹确认后治理。确定性门禁为 128 个场景、35 项指标；本阶段无迁移，不引入或连接 Codex。

补充进展（2026-09-04）：Phase 4P 已将 Phase 4N 指纹精确匹配为旧模型拍平的单个客户干系人对象。迁移 `049_v1_49` 只处理七个固定键齐全且值均为字符串的记录，在同一事务内先写不参与 API 序列化的私有隔离列、再从活跃聚合移除；不处理部分匹配或异常值，不把未经人工确认的模型内容晋升为正式联系人。降级仅恢复缺失键且不覆盖新值，部署前必须完成已验证数据库备份；无正文报告仅统计隔离完整性。确定性门禁为 129 个场景、35 项指标，不引入或连接 Codex。
...
补充进展（2026-09-05）：Phase 4S 通过 `052_v1_52` 将项目重建覆盖信息和客户来源项目集合迁入 owner 私有原生列。当前聚合 JSON 不再保存 `_coverage` / `source_project_ids`，公共 API 仍从原生列水合原字段；新快照保留这两个公共投影字段，使差异查看与回滚语义不变。迁移仅在旧值类型合法、原生为空可无损回填或双方一致时移除旧键，异常与分歧原样保留；运行期原生优先并为未迁移记录保留有界旧值回退。部署报告新增无正文 projection-state 覆盖、缺失、分歧、异常和 legacy 残留计数。确定性门禁为 132 个场景、35 项指标，不引入或连接 Codex。

补充进展（2026-09-06）：Phase 4T 基于 4S 部署后的生产报告（34 个项目、18 个客户，缺失、分歧、异常与 legacy 残留均为 0）将 `_coverage` / `source_project_ids` 切为 owner 原生列单一运行时读取权威。空原生值和异常原生 JSON 均按安全空值处理，不再从聚合副本复活数据；旧键扫描只保留在无正文诊断中。`Memory Projection Authority Report v2` 显式报告 `native_only` 与回退关闭状态，仍可发现异常回流并阻止聚合退役。本阶段不改数据库结构，唯一迁移 head 保持 `052_v1_52`；确定性门禁为 133 个场景、35 项指标，不引入或连接 Codex。
...
补充进展（2026-09-03）：Phase 4D 将 48 个内置 Skill 的 311 个 Deliverable Catalog 条目从不可变发布 Markdown 解析为结构化合同，并在项目对话提供一次性交付物选择。服务端以条目、目录和 Skill 发布三层 SHA-256 重新核对，拒绝过期选择；合同进入 Provider 边界、Message metadata、Context Receipt，并在真实 `GeneratedFile` 上持久化，打通“选择—运行—制品”的审计身份。技术校验通过的制品可由用户显式保存到项目文档，最终写入重做权限和字节 SHA 核对，持久化项目文件、操作者和时间且重试幂等；不自动写知识库、记忆或外发，仅将旧的派生项目记忆标记为待重建。幂等迁移 `045_v1_45` 保持单一 head；确定性门禁为 104 个场景、30 项指标；不引入 Codex 运行时、SDK、协议、进程或通信。

补充进展（2026-09-03）：Phase 4E 将格式对应的 Aria 声明式结构校验规则写入全部 311 个交付物合同与真实制品，多格式合同只执行实际文件类型的有界规则，失败或不可应用均阻止最终交付；语义验收仍由人工完成。制品归档必须由用户显式选择已有可写 Knowledge Source，服务端对制品与来源分别做最终授权、字节 SHA-256 和验收门禁校验，以持久归档关系及知识索引任务实现幂等可恢复处理。动作不写项目/客户记忆、不外发、不关闭问题。幂等迁移 046_v1_46 保持单一 head；确定性门禁为 108 个场景、31 项指标；不引入 Codex 运行时、SDK、协议、进程或通信。
...
### 14.1 非流式 Provider 完成状态合同（2026-09-20）

`app/services/model_completion.py` 统一校验非流式最终输出，覆盖 Kimi、DeepSeek、MiMo、BigModel 和 Claude（HTTP/SDK）。内部推理字段永远不补作最终答案；供应商声明达到 token 上限时，即使正文恰好可解析为 JSON，也按 `model_completion_truncated` 拒绝。明确拒绝/过滤/暂停及没有最终内容的响应同样失败关闭。为兼容旧代理，缺省完成原因时仍允许合法最终内容；因此未报告的截断仍依赖下游业务结构验证。

工具调用必须具备有效名称、唯一 ID 和完整的对象参数。损坏、重复键及非有限数值不再被静默改写为空对象；同一响应中任一调用无效，整份响应失败，不返回已解析的局部文本或调用。所有校验错误使用固定错误代码，不包含推理或参数正文。Claude SDK 与 HTTP 现在返回同样的多文本块/工具调用表示。
