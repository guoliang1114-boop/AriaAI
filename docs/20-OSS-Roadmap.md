...
Contract baseline validated on 2026-09-20: all 17 native Product Run event
types have a frontend runtime validator and a shared synthetic wire fixture
generated by the Python builders. CI checks Python fixture freshness,
TypeScript compatibility, lifecycle folding, and terminal failure handling.
Skill deliverable, artifact verification, approval, and pending memory
candidate receipts remain traceable while quiet chat has no step timeline.
See [the acceptance record](27-Run-Harness合同兼容性验收.md) for release evidence
and the scope of [#70](https://github.com/guoliang1114-boop/AriaAI/issues/70).
