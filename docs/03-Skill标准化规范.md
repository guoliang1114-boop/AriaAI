...
> **⚠️ 文档状态说明**  
> 本文档（03）定义 Skill 新增/维护的流程与架构视角。关于 **Skill 编写规范**（目录结构、YAML 头部、正文模板、质量门禁等），请参见 [09-Skill编写规范 v1.0](./09-Skill编写规范.md)，后者为权威规范。  
> 两份文档的关系：03 侧重"流程与架构"，09 侧重"内容与质量标准"。新增或修改 Skill 时，**两份文档均需参考**。

> 更新日期：2026-05-24  
> 关联文档：[02-Skill体系](./02-Skill体系.md) | [09-Skill编写规范](./09-Skill编写规范.md)
...
当前运行时会从冻结 Skill 发布中识别验证清单，以 SHA-256 绑定精确计划，并在交付物持久化后执行 Aria 自有的文件身份与格式完整性检查。无法映射到 Aria 声明式白名单校验器的语义/业务步骤保持人工验收；人工接受或退回必须填写依据并进入可追溯审计，不能覆盖技术失败或上下文不完整。
