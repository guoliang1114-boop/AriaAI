...
- 生成物卡片与下载
- **HITAS 人机回环审批**：服务端持久化冻结工具参数，用户确认后确定性执行；支持批次级审批（batch confirm）；单例 claim 防止并发双执行；角色化授权（viewer 不可 confirm destructive）
...
| 迁移治理 | ✅ 已完成基础闭环 | 已接入部署脚本和设置页，最新 014_v1_14 |
...
9. 安全基线：Markdown/XSS 系统复核、管理员接口权限
