...
**AriaAI**是一个面向咨询顾问和专业服务团队的 AI 工作台，核心目标不是“聊天”，而是围绕项目、客户、知识、技能和交付物组织完整工作流。

当前产品形态包括：

- 一个基于 **SwiftUI** 的 macOS 客户端
- 一个基于 **React + TypeScript + Vite** 的 Web 客户端
...
- 登录认证与基础用户管理
- 多设备登录 token 管理与基础会话失效处理
- 项目管理、里程碑、项目文件、项目文件夹、项目财务
- 客户管理与知识文档关联
- 技能管理、默认技能种子、进阶技能种子、工具定义校验
- 知识库上传、解析、切块、向量化、检索
- 聊天工作区、SSE 流式输出、Claude 工具调用
- 项目工作区单请求明细加载、项目内会话列表与首条会话自动加载
- 项目上下文摘要、项目笔记沉淀、项目文件摘要
- 生成文件管理与下载
- 模板上传与管理
- APScheduler 定时任务
- 设置管理，包括 Claude API Key 和自定义 API Base URL / HTTP 模式
- Web 端的登录、项目、聊天、知识库、技能、设置等第一版页面

### 1.3 当前开发进度判断

结合最近代码提交和已落地模块，当前项目大致处于：

> **从“多端项目工作流打磨期”进入“多端聊天体验与工程收敛并行期”**

更具体地说：

- 基础产品骨架已经完整
  - 认证、项目、聊天、知识库、技能、模板、调度、客户等主模块都已成形
  - `aria-web` 第一版已入库，产品已从单端走向双端
- 最近主线进一步转向 Web 聊天体验与多端一致性
  - 最近这轮远端更新集中在 `aria-web/src/pages/chat/Chat.tsx`、`MarkdownRenderer.tsx`、`index.css`
  - Web 端聊天已经具备分页加载、流式中断、上下文选择、复制消息、折叠侧边栏等第一版完整体验
- 工程收敛问题开始更突出
  - 仓库中出现了真实源码乱码、Web 端 README/doc 文档乱码、以及大量生成文件直接进入版本库
  - 说明项目已不再缺“页面和能力”，而更缺“编码规范、仓库边界和跨端收敛”

### 1.4 命名说明

- 产品名：`AriaAI`
- 当前主应用目录：`AriaAI`
- 后端 FastAPI title：`AriaAI API`
- Swift Package name：`Aria AI`

当前仓库已经统一迁移到 `AriaAI` 命名，代码、目录和脚本不再保留旧品牌残留。
...
```text
macOS SwiftUI App        Web App (React + Vite)
  -> APIClient.swift       -> axios API client
            \             /
             -> FastAPI backend
                -> auth middleware + token cache
                -> routers
                -> services
                -> SQLModel models
                -> PostgreSQL / SQLite-compatible model layer
                -> Claude / Kimi provider selection
...
| 层 | 技术 | 当前状态 |
|---|---|---|
| 前端（桌面） | SwiftUI, AppKit | macOS 客户端主应用 |
| 前端（网页） | React, TypeScript, Vite, Axios | `aria-web` 第一版已入库 |
| Swift 包管理 | Swift Package Manager | `Package.swift` |
| 后端 | FastAPI, Uvicorn | 主 API 服务 |
| ORM / Model | SQLModel | 全部核心表定义在 `app/models/db.py` |
| 数据库 | SQLite 本地默认，PostgreSQL 可通过环境变量覆盖 | 当前默认配置是 SQLite |
...
| 会话认证 | `UserToken` + 内存 token cache | 支持多设备登录与鉴权加速 |

### 2.3 数据库现状

这一点需要按当前代码重新理解：

- `AriaAI/backend/app/config.py` 中默认 `DATABASE_URL` 已是 **sqlite:///./data/ariaai.db**
- `start.sh` / `start.ps1` / `start.cmd` 都优先读取外部 `DATABASE_URL`，否则回退到本地 SQLite
- `requirements.txt` 仍包含 `psycopg2-binary` 和 `alembic`，说明 PostgreSQL 仍是可选生产配置
- `database.py` 里已增加本地 SQLite 轻量补列迁移逻辑，用于兼容开发期旧库

因此，当前更准确的表述是：

> **本地开发默认 SQLite，生产环境可通过环境变量切换到 PostgreSQL。**
...
当前前端已经不是单一 macOS 客户端，而是：

- `AriaAI/AriaAI`：SwiftUI 桌面主端
- `aria-web`：React + Vite 网页端

两端都直接调用同一套 FastAPI 接口，因此接下来最关键的不是“再做多少页面”，而是：
...
入口文件：`AriaAI/AriaAI/AriaAIApp.swift`
...
- `DataStore.swift`
  ...
  - 最近已新增项目明细聚合接口调用、项目内会话按项目筛选、乐观创建会话等逻辑
...
- 路由入口 `App.tsx`
- Axios API 客户端 `api/client.ts`
- 登录页、欢迎页、项目、聊天、知识库、技能、设置等页面
- Markdown 渲染组件和基础布局组件

当前 Web 端不是静态原型，而是已经进入“可联调的第一版客户端”阶段，但仍有这些特点：

- 页面覆盖面已经较广
- 状态管理仍以页面内状态为主，尚未形成统一 store
- 与早期规划文档相比，实际实现已经明显前进

### 4.6 最近前端进展

最近几次提交显示，前端重点正在从“项目工作区首屏”扩展到“Web 端聊天体验”：

- 打开项目工作区时自动加载该项目的第一条会话和消息
- 通过 `/projects/{id}/detail` 减少项目页多次往返请求
- 项目右侧信息栏开始承载上下文摘要、财务、AI 建议等项目级信息
- 聊天动画和切换体验被简化，整体方向偏向“更像工作台而不是演示型聊天”
- Web 端聊天页已经支持：
  - 会话分组
  - 搜索侧边栏
  - 分页加载历史消息
  - 中断生成
  - Project / Skill 上下文 pill
  - Markdown 代码块复制
- Web 端整体视觉系统也开始形成统一主题，而不再只是基础页面拼装
- 认证链路已演进到多设备 token + 服务端 token cache
...
- `main.py` 中新增 `_TOKEN_CACHE`，尝试减少频繁远端数据库鉴权查询
...
最近后端开发重点集中在项目工作流增强与多端支撑：

- 新增 Alembic 迁移 `001_v1_1_project_context.py`
  - 为 `Project` 增加 `context_summary` 和 `notes`
  - 为 `ProjectFile` 增加 `summary`
- `projects.py` 新增聚合接口 `/projects/{project_id}/detail`
  - 将项目、文件、里程碑、文件夹、财务明细合并为单次请求
- `projects.py` 和 `chat.py` 已开始把项目摘要、项目笔记、文件摘要注入项目工作流
- `auth.py` 和 `main.py` 新增 `UserToken` 与 token cache
  - 说明系统开始从“单端单 token”走向“多设备登录”
- `chat.py` 已支持消息分页参数 `before_id`
  - 说明聊天记录正在从“一次性全量加载”演进为更适合长会话的链路
- 这说明后端主线正在从“基础 CRUD”转向“项目上下文服务层”
...
1. 优先读取外部传入的 `DATABASE_URL`，否则使用脚本中的兼容默认值
2. 创建 `.venv`
3. 检查 Python 版本
4. 安装依赖
5. 启动 Uvicorn
...
- 后端依赖当前要求 **Python 3.9 到 3.12**
- 主要原因是 `fastembed==0.4.2` 对 Python 3.14 不兼容
- Windows 下建议优先使用 `start.ps1`
- Git Bash / MSYS 环境下可以使用 `start.sh`
- 本地开发默认数据库是 `AriaAI/backend/data/ariaai.db`
- 生产环境应通过环境变量覆盖 `DATABASE_URL`
...
- 文案和注释中仍有真实乱码，而且已经进入源码和文档
  - `aria-web/src/pages/chat/Chat.tsx`
  - `aria-web/README.md`
  - `docs/04-网页版本开发计划.md`
- 仓库中已出现直接提交的大型生成文件与项目上传文件
  - `backend/data/uploads/generated`
  - `backend/data/uploads/projects/*`
  - 这说明“运行产物是否入库”的仓库边界还未收紧
- `skills.py` 中内置技能数据量很大，维护时要注意不要误改 JSON/字符串结构
- 前端和后端已统一使用 `AriaAI` 命名
- 数据库默认配置已重新回到 SQLite 本地开发模式，但生产环境切换策略仍需继续明确
- macOS 与 Web 已进入双端并行阶段，但状态管理与接口契约还没有完全收敛
- `DataStore.loadAll()`、技能 seed/migrate、项目名称反查 ID 等历史问题仍未完全退出主链路
- 后端启动链路对 Python 版本有明确要求，当前并不是“任意安装一个最新版 Python 都能跑”
...
> **定义文件**: `AriaAI/backend/app/routers/skills.py`
...
- `AriaAI/backend/FUNCTION_CALLING.md`
- `AriaAI/TODO.md`
...
- 补充 2026-03-27 到 2026-03-28 的最新开发进展
- 更新项目工作区、项目上下文和聚合接口的现状判断
- 修正顶层仓库结构描述，改为当前实际目录
- 将文档定位从“品牌迁移说明”进一步收敛为“当前代码状态说明”

如果后续项目上下文、项目记忆和交付物链路继续推进，这份文档也需要同步更新。
