...
- 下一阶段主线已经开始收敛
  - 最近迭代重点明显集中在 `ProjectSpaceView`、`chat.py`、`projects.py`、`auth.py` 和 `aria-web`
  - 说明产品重心正在从“功能铺设”转向“项目上下文 + 项目工作区体验 + 多端对齐”
- 交付闭环还没有完全打透
  - 虽然已经具备文件生成、知识引用和项目沉淀能力
  - 但版本管理、结构化上下文组装、引用可解释性、任务状态统一等仍在早中期阶段
...
- `AriaAI/backend/app/config.py` 中的默认 `DATABASE_URL` 是 **PostgreSQL**
- `AriaAI/backend/data/ariaai.db` 仍存在，但更像兼容遗留或本地数据文件
...
最近几次提交显示，前端重点在打磨 `ProjectSpaceView` 和聊天体验：
...
- 新增 Web 端聊天、项目、技能、知识库、设置页面
- 聊天历史开始支持分页 / load more
