  ...
  const changelog: ChangelogEntry[] = [
    {
      ...
      changes: isZh
        ? [
            ...
            '记忆读取权威报告量化槽位账本完整性、待清理聚合副本和差异；部署与生产 E2E 执行无正文全库审计。',
            '已验证备份后的幂等迁移仅将历史缺失数组字段的 null 槽位占位收敛为 []，不覆盖显式空值、内容、异常摘要或版本冲突。',
            '项目/客户业务记忆已切换为槽位账本单一读取权威；缺失或损坏不会从聚合 JSON 复活旧内容。',
            '记忆审计按项目/客户作用域识别 Aria 运维元数据，值不外泄，错误作用域与未知键仍保持告警。',
            '记忆失败、晋升、取消、重建历史、覆盖与来源投影均已切为 owner 原生列单一运行时权威。',
        ...
        : [
            ...
            'A content-free memory read-authority audit now quantifies slot-ledger integrity, residual aggregate copies, and divergence during deploys and production E2E.',
            ...
            'Project and client business memory now reads exclusively from digest-verified slot ledgers; missing or corrupt slots cannot revive aggregate JSON content.',
            ...
            'Memory failures, promotions, cancellations, rebuild history, coverage, and source projections now use owner-native columns as their sole runtime authority.',
