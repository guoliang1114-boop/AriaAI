...
export function ProjectChatActionPreviewPanel({
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-end justify-center bg-codex-ink/20 px-4 py-5 backdrop-blur-[2px] sm:items-center">
      <div className="w-full max-w-4xl overflow-hidden rounded-xl border border-codex-line bg-white shadow-[0_24px_80px_rgba(15,23,42,0.25)]">
      <div className="flex flex-wrap items-center gap-3 border-b border-codex-line-soft bg-codex-bg-tint/80 px-4 py-3">
        <div className={`flex h-9 w-9 items-center justify-center rounded-lg ${kind.tone === "rose" ? "bg-codex-bg-tint text-codex-bad" : "bg-codex-bg-tint text-codex-warn"}`}>
          {kind.icon}
        </div>
        ...
        <div className="inline-flex items-center gap-1.5 rounded-full border border-codex-line bg-white px-2.5 py-1 text-xs font-medium text-codex-warn">
        ...
        <button
          type="button"
          onClick={onCancel}
          className="rounded-lg p-1.5 text-codex-ink-faint transition hover:bg-white hover:text-codex-ink-soft"
      ...
      <div className="grid gap-3 px-4 py-3 md:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-lg border border-codex-line-soft bg-codex-bg-tint/80 px-3 py-2.5">
          <div className="mb-2 flex items-center gap-1.5 text-xs font-semibold text-codex-ink-soft">
            <AlertTriangle className="h-3.5 w-3.5 text-codex-warn" />
            {isZh ? "将执行的动作" : "Action to run"}
          </div>
          <p className="text-sm font-medium text-codex-ink">{kind.label}</p>
          {call.summary || call.message ? (
            <p className="mt-1 text-xs leading-5 text-codex-ink-mute">{call.summary || call.message}</p>
          ) : null}
          <p className="mt-2 text-xs leading-5 text-codex-ink-faint">
            {canConfirm
              ? isZh
        ...
        <div className="rounded-lg border border-codex-line-soft bg-codex-bg-tint/80 px-3 py-2.5">
          <p className="mb-2 text-xs font-semibold text-codex-ink-soft">
            {isZh ? "影响范围" : "Impact"}
          </p>
          {details.length ? (
            <div className="max-h-24 space-y-1 overflow-y-auto pr-1">
              {details.map((detail, index) => (
                <p key={`${detail}-${index}`} className="text-xs leading-5 text-codex-ink-soft">
                  {detail}
                </p>
              ))}
            </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-codex-line-soft px-4 py-3">
        <p className="text-xs leading-5 text-codex-ink-mute">
          {isZh
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={onCancel}
            className="rounded-lg border border-codex-line bg-white px-3 py-1.5 text-sm font-medium text-codex-ink-soft transition hover:bg-codex-bg-tint"
          >
            {isZh ? "取消" : "Cancel"}
          </button>
          {canConfirm ? (
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg bg-codex-ink px-3 py-1.5 text-sm font-semibold text-white transition hover:bg-codex-ink disabled:cursor-not-allowed disabled:opacity-50"
            >
