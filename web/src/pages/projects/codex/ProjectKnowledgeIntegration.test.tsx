...
describe('native project knowledge scope integration', () => {
  ...
  it('carries restored documents on actual composer sends and drops them after explicit clear', async () => {
    mount()
    expect(await screen.findByRole('button', { name: '管理知识范围' })).toHaveTextContent('IBM 方法文档')
    expect(screen.queryByText(/移除最后一份资料后/)).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: '清空资料，恢复项目默认范围' })).not.toBeInTheDocument()
    enter('精简成两条')
    await waitFor(() => expect(mocks.send).toHaveBeenCalledWith('精简成两条', { knowledgeDocumentIds: [7] }))
    fireEvent.click(screen.getByRole('button', { name: '管理知识范围' }))
  ...
  it('adds a new document in the native picker without auto-sending or losing the draft', async () => {
    ...
    expect(screen.getByRole('button', { name: '管理知识范围' })).toHaveTextContent('新资料')
  ...
  it('does not carry one conversation selection into another', async () => {
    mount()
    fireEvent.click(await screen.findByRole('button', { name: '管理知识范围' }))
    expect(screen.getByRole('button', { name: '移除知识文档 IBM 方法文档' })).toBeVisible()
    ...
    await waitFor(() => expect(screen.getByRole('button', { name: '管理知识范围' })).toHaveTextContent('项目默认'))
    expect(screen.getByRole('button', { name: '管理知识范围' })).toHaveAttribute('aria-expanded', 'false')
    enter('这个项目进展如何')
