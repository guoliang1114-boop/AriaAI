...
function SectionList({
  ...
  if (items.length === 0) {
    return <div className="rounded-lg bg-codex-bg-tint p-3 text-sm text-codex-ink-mute">{emptyText}</div>;
  }
    ...
    <div className="rounded-xl border border-codex-line bg-white p-5">
      <div className="mb-4 flex items-center gap-2">
        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-codex-bg-tint">
          <Icon className="h-4 w-4 text-codex-ink-soft" />
        </div>
...
function StakeholderManagementCard({
  ...
  return (
    <div className="rounded-xl border border-codex-line-soft bg-gradient-to-br from-sky-50 via-white to-emerald-50 p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-semibold text-codex-ink">{isZh ? "干系人管理与分析" : "Stakeholder Management & Analysis"}</h3>
              <p className="mt-0.5 text-sm text-codex-ink-mute">
                {isZh
        ...
        <button
          ...
          disabled={!client}
          className="rounded-lg border border-codex-line bg-white px-3 py-2 text-sm font-medium text-codex-accent-ink hover:bg-codex-accent-bg"
        >
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-3">
        <div className="rounded-xl border border-codex-line-soft bg-white/80 p-4">
          <div className="text-xs font-medium text-codex-ink-mute">{isZh ? "关联客户" : "Linked Client"}</div>
          <div className="mt-2 text-xl font-semibold text-codex-ink">{client?.name || (isZh ? "未关联" : "Not linked")}</div>
          <div className="mt-2 text-sm text-codex-ink-soft">
            {client?.contact || client?.industry || (isZh ? "建议先在项目设置中关联客户" : "Link a client in project settings first")}
          </div>
        </div>
        <div className="rounded-xl border border-codex-line-soft bg-white/80 p-4">
          <div className="text-xs font-medium text-codex-ink-mute">{isZh ? "客户关键联系人" : "Client key contacts"}</div>
          <div className="mt-2 text-2xl font-semibold text-codex-ink">{clientContacts.length}</div>
          <p className="mt-2 text-sm text-codex-ink-soft">
        ...
        <div className="rounded-xl border border-codex-line-soft bg-white/80 p-4">
          <div className="text-xs font-medium text-codex-ink-mute">{isZh ? "分析结论" : "Analysis"}</div>
          <p className="mt-2 text-sm text-codex-ink-soft">
      ...
      <div className="mt-4 rounded-xl border border-codex-line-soft bg-white/70 p-4">
        <div className="text-xs font-medium text-codex-ink-mute">{isZh ? "固定沟通提醒" : "Pinned communication reminders"}</div>
        {pinnedNotes.length ? (
          <ul className="mt-2 grid gap-2 text-sm text-codex-ink-soft md:grid-cols-2">
      ...
      {aiNotes.length ? (
        <div className="mt-4 rounded-xl border border-codex-line-soft bg-white/70 p-4">
          <div className="text-xs font-medium text-codex-ink-mute">{isZh ? "AI 干系人观察" : "AI stakeholder observations"}</div>
          <ul className="mt-2 grid gap-2 text-sm text-codex-ink-soft md:grid-cols-2">
            {aiNotes.slice(0, 4).map((item, index) => (
              <li key={`${item}-${index}`} className="flex items-start gap-2">
                <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-codex-accent" />
                <span>{item}</span>
              </li>
            ))}
          </ul>
...
export function ProjectMemoryTab({ projectDetail, projectId }: ProjectMemoryTabProps) {
  ...
  return (
    <div className="space-y-6">
      {promotionMeta ? (
        <div className="rounded-xl border border-codex-line bg-codex-accent-bg p-4 text-sm text-codex-good">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <div className="mt-1 text-codex-good">
                {isZh
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-lg border border-codex-line bg-white px-3 py-2 text-sm font-medium text-codex-good hover:bg-codex-accent-bg"
            >
              {isZh ? "打开客户记忆" : "Open client memory"}
        ...
        <div className="rounded-xl border border-codex-line bg-gradient-to-br from-teal-50 via-emerald-50 to-white p-4 text-sm text-codex-good">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 max-w-3xl text-codex-good">
                {isZh
                  ? clientName
              ...
              {!hasProjectMemory ? (
                <p className="mt-2 text-xs font-medium text-codex-warn">
                  {isZh ? "建议先刷新项目记忆，再沉淀到客户记忆。" : "Rebuild project memory before promoting it."}
                </p>
              ) : null}
              {clientPromotionError ? (
                <p className="mt-2 text-xs font-medium text-codex-bad">{clientPromotionError}</p>
              ) : null}
              {clientPromotionMessage ? (
                <p className="mt-2 text-xs font-medium text-codex-good">{clientPromotionMessage}</p>
              ) : null}
            </div>
            <button
              type="button"
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:col-span-2 xl:grid-cols-4">
          ...
          <div className="rounded-xl border border-codex-line bg-white p-5">
            ...
            <div className="mt-2 text-sm leading-relaxed text-codex-ink">
          ...
          <div className="rounded-xl border border-codex-line bg-white p-5">
            ...
            <div className="mt-2 text-sm font-medium text-codex-ink">{memoryUpdatedText}</div>
          </div>
          <div className="rounded-xl border border-codex-line bg-white p-5">
            <div className="text-xs font-medium text-codex-ink-mute">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-codex-ink-soft">
              {sourceCoverage.map((item) => (
                <div key={item.label} className="rounded-lg bg-codex-bg-tint px-2.5 py-2">
                  <div>{item.label}</div>
                  <div className="mt-1 font-semibold text-codex-ink">{item.value}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {memory?.stale ? (
        <div className="rounded-xl border border-codex-line bg-codex-bg-tint p-4 text-sm text-codex-warn">
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <div className="mt-1 text-codex-warn">
                {isZh
      ...
      <div className="rounded-xl border border-codex-line bg-white p-4">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 text-sm">
          <div>
            ...
            <div className="mt-1 font-semibold text-codex-ink">{rebuildStatusText}</div>
          </div>
          <div>
            <div className="text-xs font-medium text-codex-ink-mute">
              {isZh ? "最近触发" : "Last Trigger"}
            </div>
            <div className="mt-1 font-semibold text-codex-ink">{lastTrigger}</div>
          </div>
          <div>
            <div className="text-xs font-medium text-codex-ink-mute">
              {isZh ? "最近失败" : "Last Failed"}
            </div>
            <div className="mt-1 font-semibold text-codex-ink">
      ...
      <div className="rounded-xl border border-codex-line-soft bg-gradient-to-br from-indigo-50 via-white to-sky-50 p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <div className="flex items-center gap-2 font-semibold text-codex-ink">
              <Clock3 className="h-4 w-4 text-codex-accent" />
              {isZh ? "记忆历史版本" : "Memory history"}
            </div>
            <p className="mt-1 text-sm leading-6 text-codex-ink-soft">
              {isZh
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-lg border border-codex-line bg-white px-3 py-2 text-sm font-medium text-codex-accent-ink hover:bg-codex-accent-bg"
          >
        ...
        {snapshots.length ? (
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {snapshots.slice(0, 6).map((snapshot) => {
              ...
              return (
                <div key={snapshot.id} className="rounded-xl border border-codex-line-soft bg-white/85 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-1 text-xs text-codex-ink-mute">{formatProjectMemoryUpdatedAt(snapshot.created_at, isZh)}</div>
                    </div>
                    {isCurrent ? (
                      <span className="rounded-full bg-codex-accent-bg px-2 py-1 text-xs text-codex-good">
                        {isZh ? "当前" : "Current"}
                      </span>
                    ) : null}
                  </div>
                  <div className="mt-3 rounded-lg bg-codex-accent-bg px-3 py-2 text-xs text-codex-accent-ink">
                  ...
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    <button
                      ...
                      className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-codex-line bg-white px-3 py-2 text-sm font-medium text-codex-accent-ink hover:bg-codex-accent-bg disabled:cursor-wait disabled:text-codex-ink-faint"
                    >
                    ...
                    <button
                      ...
                      className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-codex-line bg-white px-3 py-2 text-sm font-medium text-codex-accent-ink hover:bg-codex-accent-bg disabled:cursor-not-allowed disabled:border-codex-line disabled:text-codex-ink-faint"
                    >
          ...
          <div className="mt-4 rounded-xl border border-dashed border-codex-line bg-white/70 p-4 text-sm text-codex-ink-mute">
            {isZh ? "暂无记忆历史。下一次刷新项目记忆后会自动生成快照。" : "No memory history yet. The next rebuild will create a snapshot automatically."}
          </div>
        )}
        {snapshotDiff ? (
          <div className="mt-4 rounded-xl border border-codex-line-soft bg-white p-4 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="text-sm font-semibold text-codex-ink">
                  {isZh
            ...
            <div className="mt-4 space-y-3">
              {snapshotDiff.fields.length ? (
                snapshotDiff.fields.slice(0, 8).map((field) => (
                  <div key={field.field} className="rounded-lg border border-codex-line-soft bg-codex-bg-tint p-3">
                    <div className="text-xs font-semibold text-codex-ink-mute">{field.label}</div>
                    {field.kind === "list" ? (
                      <div className="mt-2 grid gap-2 text-sm md:grid-cols-2">
                        <div className="rounded-md bg-codex-accent-bg p-2 text-codex-good">
                        ...
                        <div className="rounded-md bg-codex-bg-tint p-2 text-codex-bad">
                ...
                <div className="rounded-lg bg-codex-bg-tint p-3 text-sm text-codex-ink-mute">{isZh ? "这个快照与当前记忆没有可见差异。" : "No visible differences from current memory."}</div>
              )}
            </div>
          </div>
        ) : null}
      ...
      {rollbackConfirmSnapshot ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-codex-ink/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="text-base font-semibold text-codex-ink">{isZh ? "确认恢复项目记忆？" : "Restore project memory?"}</h3>
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button
                ...
                className="rounded-lg border border-codex-line px-4 py-2 text-sm font-medium text-codex-ink-soft hover:bg-codex-bg-tint"
              >
                {isZh ? "取消" : "Cancel"}
              </button>
              <button
        ...
        <DetailCard icon={Brain} title={isZh ? "项目概况" : "Project Brief"}>
          <div className="space-y-3">
            <div>
              ...
              <div className="mt-2 text-sm leading-relaxed text-codex-ink-soft">
        ...
        <DetailCard icon={FileText} title={isZh ? "重要文档与财务状态" : "Documents & Financial Status"}>
          <div className="space-y-5">
              ...
              {memory?.important_documents?.length ? (
                <div className="space-y-3">
                  {memory.important_documents.map((document, index) => (
                    <div key={`${document.name}-${index}`} className="rounded-lg bg-codex-bg-tint p-3">
                      <div className="text-sm font-medium text-codex-ink">{document.name}</div>
                      <div className="mt-1 text-sm text-codex-ink-soft">{document.reason}</div>
                    </div>
                  ))}
                </div>
              ) : (
            ...
            <div>
              ...
              <div className="rounded-lg bg-codex-bg-tint p-3 text-sm leading-relaxed text-codex-ink-soft">
