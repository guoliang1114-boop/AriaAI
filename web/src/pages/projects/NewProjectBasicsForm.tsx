  ...
  return (
    <>
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-codex-ink-mute">{isZh ? '基本信息' : 'Basics'}</span>
        <div className="flex-1 h-px bg-outline/10" />
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <div className="relative">
            <Briefcase className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-codex-ink-mute pointer-events-none" />
            <input
              type="text"
              required
              value={formData.name}
              ...
              className="w-full pl-10 pr-4 py-3 bg-codex-bg-tint-lowest rounded-xl border border-outline/15 text-sm text-codex-ink placeholder:text-codex-ink-mute outline-none focus:border-primary/30 focus:ring-2 focus:ring-primary/10 transition-colors"
            />
          </div>
        </div>
      ...
      <div>
        ...
        <div className="relative">
          <FileText className="absolute left-3.5 top-3.5 w-4 h-4 text-codex-ink-mute pointer-events-none" />
          <textarea
            rows={3}
            value={formData.description}
            onChange={e => onFieldChange('description', e.target.value)}
            placeholder={isZh ? '简要描述项目背景、目标和范围...' : 'Describe the project background, goals, and scope...'}
            className="w-full pl-10 pr-4 py-3 bg-codex-bg-tint-lowest rounded-xl border border-outline/15 text-sm text-codex-ink placeholder:text-codex-ink-mute outline-none focus:border-primary/30 focus:ring-2 focus:ring-primary/10 transition-colors resize-none"
          />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="relative">
          <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-sm text-codex-ink-mute font-medium">¥</span>
          <input
            type="number"
            min="0"
            ...
            placeholder="0"
            className="w-full pl-8 pr-4 py-3 bg-codex-bg-tint-lowest rounded-xl border border-outline/15 text-sm text-codex-ink placeholder:text-codex-ink-mute outline-none focus:border-primary/30 focus:ring-2 focus:ring-primary/10 transition-colors"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="flex-1 h-px bg-outline/10" />
        <span className="text-xs text-codex-ink-mute">{isZh ? '项目阶段' : 'Stage'}</span>
        <div className="flex-1 h-px bg-outline/10" />
      </div>
      ...
      <div className="flex items-center justify-end gap-3 pt-2 border-t border-outline/10">
        <button
          type="button"
          onClick={onCancel}
          className="px-5 py-2.5 text-sm text-codex-ink-mute hover:bg-codex-bg-tint rounded-xl transition-colors"
        >
          {isZh ? '取消' : 'Cancel'}
        </button>
        <button
